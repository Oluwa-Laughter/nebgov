...
if (invalidFiles.length > 0 || collisions.length > 0) {
  console.error("Invalid indexer migration numbering:");
  for (const file of invalidFiles) {
    console.error(`- ${file} does not start with a three-digit prefix`);
  }
  for (const [prefix, files] of collisions) {
    console.error(`- prefix ${prefix} is used by: ${files.join(", ")}`);
  }
  process.exit(1);
}
